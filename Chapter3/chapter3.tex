...
\nomenclature[s-i]{\textit{i}}{Property of the computational node}
\nomenclature[a-x]{\textit{p}}{Position of a material point / node}
\nomenclature[a-V]{\textit{V}}{Volume}
\nomenclature[a-w]{\textit{w}}{Weight function / test function}
\nomenclature[a-N]{\textit{N}}{Shape function}
\nomenclature[a-B]{\textit{B}}{Gradient of the function}
\nomenclature[g-Omega]{$\Omega$}{Domain under consideration}
\nomenclature[g-epsilon]{$\epsilon$}{Strain component}
...
continuum, $\tau$ is the surface traction. Eq~\ref{eq:weak} is obtained by 
...
\If {$a_{sI,t}^{k+1} < 0.0$}
    ...
    \If {$v_{sI,t}^{k+1,tmp}= 0$} \Comment{static fricion condition}
        ...
        \EndIf
    \Else  \Comment{Kinetic fricion condition}
        \If {$v_{sI,t}^{k+1} \leq \mu_k \left| a_{sI,n}^{k+1} \right| \Delta t$}
            \State $a_{sI,t}^{k+1} = - \frac{v_{sI,t}^{k}}{\Delta t}$
        \Else \Comment{friction acts in the direction opposite to the movement}
...
are solved on the nodes, resulting in updated nodal velocities and positions. 
...
cross the cell boundaries as the body deforms, this is termed \textit{cell 
...
node \textit{i} at position $x_i$ is then $\psi_i=\psi(x - x_i)$. The basis 
functions in 3-D are separable functions constructed as $\psi_i(x) = \psi_i^x(x)
\psi_i^y(y)\psi_i^z(z)$. GIMP is often implemented using the standard 
...
\nomenclature[g-psibar]{$\bar{\psi}$}{Weighting function}
where $l_p$ is the width of the particle characteristic function 
$\chi_p$.~\cref{fig:gimp} shows a 1-D GIMP weighting function $\bar{\psi}_{ip}$ 
...
\begin{figure}[tbhp]
	...
	\caption{Example GIMP weighting function $\bar{\psi}_{ip}$ ,
...
grains interact with each other. Analytical and finite element models 
...
local granular variables, like grain position, velocities, contact 
...
flow-like behaviour. They observed dilatant grain-shearing flow is 
...
reasonable results for the extent of deposits and the relative 
...
of a classical multi-body system. The Discrete Element Method is a simple and 
...
trajectories of all the grains of the system is called as the Discrete Element 
Method.

DEM simulates similar to the real experiments, involving the generation 
...
not yet fully support micro-scale fluid solid coupling using Lattice Boltzmann 
approach, which will be used to study the mechanism of submarine landslides. 
A new C++ based high performance DEM code is developed in the present study. 
The features implemented in the present study and the general overview of DEM 
are discussed below.
...
chosen to be sufficiently large that the overlap between the grains remain 
...
neighbours, at least in the following few time steps. During initialization the 
...
DEM by~\citet{Verlet1967}. The Verlet algorithm is 
...
the total energy of the system exactly~\citep{Frenkel1996}. The Verlet 
...
preserve energy and has significant energy drifts. The simplest among the 
...
shear contact model in DEM. The theoretical solution for a ball rolling down an 
...
as the ration of volume of solids to the total volume ($V_s / V$), of 
...
and/or higher numerical efficiency~\citep{Radjai2011}. For example, a dense 
...
grains. Such a packing may then be used as the initial state for dynamic 
