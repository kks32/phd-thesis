...
under the action of gravity in a viscous Newtonian fluid. It consists of the 
three following steps named \emph{draft}, \emph{kiss} and \emph{tumbling}. The 
\emph{draft}: grain 1 is first placed within the hydrodynamic drag above 
grain 2. As the hydrodynamic drag of particle 2 is a depression zone, 
grain 1 is attracted inside. The \emph{kiss}: particle 1 increases its 
vertical velocity until it touches grain 2. The horizontal velocity of 
grain 2 increases and its vertical velocity decreases below that of 
grain 1. \emph{Tumbling}: grain 1 having the same horizontal velocity and 
higher vertical velocity than grain 2, overtakes grain 2.
%
\begin{figure}[tbhp]
\centering
...
In the present study Nvidia GE Force GTX 580 GPU with 512 cuda cores is used.
...
code (kernels) run on an accelerator such as a GPU (see~\cref{fig:GPUConcept}). 
OpenACC API version 2.0 is used in the present study and has two compute 
constructs, the kernels construct and the parallel construct. LBM -- DEM 
implementation predominantly uses the OpenACC gang and vector parallelism.
