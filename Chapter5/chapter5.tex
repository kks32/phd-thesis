...
To verify the incompressible LBM flow model implemented in the above section, 
numerical simulation of the steady state Poiseuille flow was conducted. The 
transient development of Poseuille's flow in a straight channel is computed. 
Water ($\rho=1000 kg/m^{3}$) is simulated to flow through a channel of width 
`H' (= 0.04 \si{\meter}) and simulation length, `L' (2.5H). Periodic 
boundary conditions are applied at either end of the pipe and the pressure 
gradient is set to zero, which simulates the condition of a continuous flow of 
fluid in a closed circular pipe. The length has no effect on the simulation as 
no streamwise variation is detected in the solution. The parameters adopted in 
LBM simulation are summarised in~\cref{table:lbm}. Sufficient time is allowed 
for the flow to travel beyond the required development length so that the flow 
is laminar~\citet{Durst2005}. The development length required for a flow to 
be fully laminar is
%
\begin{equation}
...
presented in~\cref{fig:LBM_Poiseuille}. A maximum horizontal velocity of 
0.037863~\si{\meter\per\second} is observed. The maximum horizontal velocity is 
compared with the closed-form based on the Haygen-Poiseuille's flow equation 
for no-slip boundary condition~\citep{Willis2008}
%
\begin{equation}
...
pressure gradient, $\mu$ dynamic viscosity of the fluid.
...
The horizontal velocity profile estimated at `L/2' based on the 
...
performed using ANSYS Fluent. CFD analysis 
...
be constructed via Gram-Schmidt orthgonalisation procedure. The general form of 
...
Method modelling of the particle interaction with the LB formulation, a minor 
