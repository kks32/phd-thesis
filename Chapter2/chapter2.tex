...
Granular flows display a large span of grain concentration and therefore 
exhibit different behaviour at different concentrations (\cref{fig:kinetic}). 
In general the behaviour can be classified into three mechanisms: \textit{a}. 
in the \textit{dilute part of the flow}, grains randomly fluctuate and 
translate, this form of viscous dissipation and stress is named kinetic, 
...
stress, named \textit{collisional} and \textit{c} at very high concentration 
(more than 50\% in volume), grains start to endure long, sliding and rubbing 
contacts, which gives rise to a totally different from of dissipation and 
stress, named \textit{frictional}. The momentum and energy transfer will be 
...
component, hallow water approximation is used. The capability and limitation of 
the various frameworks are discussed below. 
%
\begin{figure}[tbhp]
\centering
...
Grains experience long and permanent contacts in rubbing, rolling on each 
other. Hence a stress tensor based on the mechanical law of friction must be 
developed. Granular flow modelling began as early as 1776 with Coulomb's paper 
describing the yielding of granular material as a frictional process. 
...
The formalisms of gas kinetic theory can be used to derive a
...
kinetic theory formalisms yield a set of Navier – Stokes like equations. 
...
contact term. Which is self-contradictory as they are formed by simply adding a 
rapid-flow model, based on instantaneous collisions, to a frictional model, 
based on long duration contacts. Also, the rapid-flow models based on gas 
...
the opening of a gate. The material flows down and develops into a dense 
...
The two curve $h_{stop}(\theta)$ and $h_{start}(\theta)$ divides the phase 
...
\subsubsubsection{Comments on modelling}
...
suggested that the main flow is governed by the momentum of the flow. It is 
...
dynamics. Experimental conditions are too difficult to reproduce precisely, 
resulting in inherent inconsistencies in the results. Many models, such as the 
shallow-water approximation, kinetic theory approach, and rheologies based on 
shear rate, have captured the basic flow dynamics, but have failed to describe 
the complete mechanics of the granular flow.  Most theoretical models are 
incapable of capturing these transition regimes.
...
flow. The initial conditions have a significant impact on the overall behaviour 
of the granular system, however, a theoretical framework that is capable of 
describing the influence of material properties on the run-out behaviour is 
still lacking. In the present study the capability of continuum models such as 
the Mohr-Coulomb model and $\mu(I)$ rheology in modelling the granular column 
collapse is investigated by comparing it with the discrete-element simulations. 
The role of initial packing fraction on the run-out behaviour is also 
...
collapse. Very few research has been carried out to understand the difference 
in mechanism of dry and submerged granular flow. The difference in behaviour 
between the dry and the submerged case is not understood. The role of initial 
volume fraction on the run-out behaviour is not precisely understood. This 
study focuses on the effect of initial packing and permeability on the run-out 
by performing grain-scale simulations. This study investigates the influence of 
grain-scale quantities, which are otherwise not accessible experimentally, on 
the run-out behaviour thus providing useful insight into the flow dynamics, 
thereby enabling us to develop better constitutive laws. 
