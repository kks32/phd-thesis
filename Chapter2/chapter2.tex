...
frictional contacts and inelastic collisions.~\Cref{fig:granular} provides a 
...
The range of grain size gives rise to complex interactions between grains 
...
The momentum and energy transfer are different in accordance with differing 
...
\nomenclature[g-phi]{$\phi$}{Friction angle in degrees}
...
\nomenclature[g-alpha]{$\alpha$}{Conductivity}
%\nomenclature[g-epsilonvar]{$\varepsilon$}{Coefficient of restitution}
\nomenclature[G-Gamma]{$\Gamma$}{Dissipation through inelastic collision}
\nomenclature[a-T]{\textit{T}}{Temperature}
\nomenclature[a-C]{\textit{C}}{Concentration in Kinetic theory}
\nomenclature[a-c]{\textit{c}}{Pressure wave velocity (or) Lattice speed in LBM}
\nomenclature[a-u]{\textit{u}}{Velocity component along \textit{x}-direction}
\nomenclature[a-v]{\textit{v}}{Velocity component along \textit{y}-direction}
\begin{equation}
...
\subsection{Rheology}
\label{sec:muI}
\nomenclature[g-sigma]{$\sigma$}{Stress or confining pressure}
\nomenclature[g-gammadot]{$\dot{\gamma}$}{Shear rate}
\nomenclature[g-tau]{$\tau$}{Shear stress (or) Surface traction (or ) 
Relaxation time in LBM}
\nomenclature[g-delta]{$\delta$}{Overlap between grains (or) Dirac function}
\nomenclature[A-I]{\textit{I}}{Inertial number (or) Moment of Inertia}
\nomenclature[r-prime]{$\prime$}{Effective component of the normal stress}
\nomenclature[s-n]{$n$}{Normal component}
\nomenclature[s-t]{$t$}{Tangential component}
\nomenclature[a-p]{\textit{p}}{Pressure}
...
number \textit{I} as
%
\nomenclature[g-mu]{$\mu$}{Coefficient of friction (or) Dynamic viscosity in 
fluid}
\nomenclature[s-0]{\textit{0}}{Initial state}
\nomenclature[s-f]{\textit{f}}{Final state}
\begin{equation}
...
To formulate a complete constitutive model, it is essential to describe the 
...
\nomenclature[a-kf]{$\mathit{k_f}$}{Dimensionless parameter defined as the ratio
 of normal stiffness $k_n$ to the applied normal pressure $\sigma_n^\prime$}
\nomenclature[a-kstar]{$k^*$}{Dimensionless parameter defined as the ratio of 
elastic to inertial effects}
\nomenclature[a-k]{$k$}{Stiffness of a grain (or) time step (or) Permeability}
\nomenclature[a-d]{\textit{d}}{Grain diameter}
...
% \citet{Campbell2002} observed a strong correlation between the coefficient of 
%friction and the dimensionless parameter, \textit{k}, as the flow progresses 
%through different regimes, similar to the observation made by 
%\citet{Kamrin2010}. 
...
\end{align}
%
\nomenclature[a-f]{\textbf{f} / \textit{F}}{Applied force (or) Vector of fluxes 
in the Navier-Stokes equation}
\nomenclature[a-g]{\textit{g}}{Acceleration due to gravity}
\nomenclature[a-t]{\textit{t}}{Time}
\nomenclature[a-h]{\textit{h}}{Vertical length scale in shallow water 
approximation (or) Length of a LBM cell}
\nomenclature[a-l]{\textit{l}}{Horizontal length scale in shallow water 
approximation (or) particle spacing in the Material Point Method}
%
\begin{figure}[tbhp]
\centering
\includegraphics[width=0.75\textwidth]{SH}
...
\nomenclature[a-L]{\textit{L}}{Length of a granular column (or) background cell 
size}
\nomenclature[a-H]{\textit{H}}{Height of a granular column}
\nomenclature[s-int]{\textit{int}}{Internal}
\nomenclature[s-ext]{\textit{ext}}{External}
\nomenclature[a-a]{\textit{a}}{Initial aspect ratio of a granular column}
\nomenclature[a-A]{\textbf{a}}{Acceleration of a grain or a material point}
\nomenclature[a-m]{\textit{m}}{Mass}
\nomenclature[g-tauc]{$\tau_c$}{Critical time}
\nomenclature[g-lambda]{$\lambda$}{Constant of proportionality in the power-law 
relationship (or) The time step factor in DEM}
...
material of mass \textit{m}.~\Cref{fig:exp} shows the schematic view of the 
...
\subsubsubsection{Experimental findings}
...
\subsubsubsection{Axisymmetric versus two-dimensional collapse}
...
\citet{Balmforth2005} observed that the constant of proportionality, $\lambda$, 
...
\subsubsubsection{Numerical modelling}
...
The final collapse height observed in the numerical simulations of granular 
...
The flow dynamics involve the spreading of granular mass 
...
Understanding the internal flow structure will provide an important insight 
...
aspect ratio of 0.4 and 3 at time $t= 0.4\tau_c$~\citep{Lajeunesse2004}.]{The 
extent of the failure surface for granular columns with initial aspect ratio of 
0.4 and 3 at time $t= 0.4\tau_c$~\citep{Lajeunesse2004}. Short columns show 
truncated conical deposit at the end of the flow, while tall columns exhibit 
conical deposit.}
\label{fig:Deposit}
\end{figure}


Identification of the static region is an important task, as the static region 
...
\subsubsubsection{Numerical findings}
...
\nomenclature[g-theta]{$\theta$}{Slope angle}
\nomenclature[a-W]{\textit{W}}{Width of the channel}
...
\nomenclature[a-Ep]{$\mathit{E_p}$}{Potential energy}
\nomenclature[a-Ek]{$\mathit{E_k}$}{Kinetic energy}
\nomenclature[a-Ef]{$\mathit{E_f}$}{Energy lost through friction}
\nomenclature[a-Ev]{$\mathit{E_v}$}{Energy lost through viscous dissipation}
\nomenclature[a-Ed]{$\mathit{E_D}$}{Energy lost through drag}
\nomenclature[a-Er]{$\mathit{E_r}$}{Energy lost due to remolding and 
transformation}
\begin{equation}
...
\Cref{fig:Runout_PWP_Fluid_Exp} demonstrates the evolution of run-out 
...
The dense column must dilate in order to flow. 
...
The collapse of a granular column is a simple case of granular flow. 
