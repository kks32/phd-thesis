...
The runout distance $R_f$ is the distance of the rightmost particles 
...
columns. The total runout duration $t_f$ is the time that the pile takes to 
reach its final runout distance $R_f$.
...
energy input $E_0$. The runout distance $R_f$ is the distance of the rightmost 
...
the experiments of collapsing columns. The total runout duration $t_f$ is the 
time that the pile takes to reach its final runout distance $R_f$.       
...
%-------------------------------------------------------------------------------------------
\subsection{Evolution of pile geometry and runout}
\label{sec:evolution}
...
long runout distance.      
The transient is more energy consuming in  mode 3  compared to 
mode 1. For this reason, the runout distance in mode 3 is long but shorter 
...
Figure \ref{fig:runout} shows the normalized runout distance $(R_f - R_0)/R_0$ 
and total runout time $t_f$ as a function of the input energy $E_0$. We observe 
two regimes both characterized by a power-law runout distance and time as a 
function of $E_0$. In the first regime, corresponding to the range of low input 
energies $E_0 < 40 \ mgd$, the runout distance varies as $R_f \propto 
...
An average runout speed can be defined from the ratio $v_s = (R_f - R_0) / 
...
during runout declines when the input energy is increased. As we shall see 
below, the constant runout time reflects also the collapse of the particles 
into the cavity left behind the pile. 

In the second regime, corresponding to the range of high input energies  $E_0 > 
40 \ mgd$, the runout distance varies as $R_f \propto (E_0)^{\alpha'}$ over one 
...
regime the average runout speed varies as $v_s \propto (E_0)^{0.56 \pm 0.07}$. 
...
runout distance and time were found in the case of 
...
Topin et al. found that the runout distance varies as a power law 
...
factor for the runout distance.
...
%\caption{Snapshots of the initial stages of runout for 
...
runout. This regular fall-off occurs clearly with two different functional 
...
regime the runout time $t_f$ is independent of the input energy. In contrast, 
the plots corresponding to the second regime (high energies) collapse only at 
the beginning of runout, i.e. for $t < t_1 \simeq 7.5 \ (d/g)^{0.5}$.   
...
is responsible for most of the runout and hence it is expected to control the 
runout distance and time. Fig. \ref{fig:ExoEx0_vs_t_totau}(a) shows the 
...
\begin{figure}[tbhp]
\centering
\includegraphics[width=\textwidth]{ExEx0_vs_ttau}
...
runout time $t'_f$ since the beginning of the second phase might be a simple 
...
nearly one decade. The runout time $t'_f \propto (E'_{x0})^{\beta'}$ has the 
same exponent $\beta' \simeq 0.21 \pm 0.03$ as $t_f$ as a function of $E_0$ 
(see Fig. \ref{fig:runout}). For the decay time we have 
...
data. It is therefore plausible to assume that  the runout time is a multiple 
...
%}}
%\caption{Linear relationship between decay time and runout time after the 
%transient (a) 
...
The runout distance and time and the dissipation of kinetic energy are 
...
the effect of the friction coefficient is quite important for the runout, as 
observed in Fig. \ref{fig:para} for both the energy decay and geometrical 
profile of the pile. Both the runout distance and decay time decrease as the 
friction coefficient is increased. This effect is much more pronounced at low 
values of the friction coefficient. The runout time, for example, is reduced by 
a factor 4 as $\mu_s$ is increased from 0.1 to 0.4 while the runout times and 
...
response to the increase of the friction coefficient \cite{Estrada_2008}.
...
runout distance and time with non-trivial exponents. This is a central result 
...
reflects mainly the destabilization of the pile by the quake with a runout time 
