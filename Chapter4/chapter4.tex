...
energy is transferred to the vertical component of the velocity field and 
...
energy evolution corresponding to low energy regime nearly collapse on to a 
...
for the kinetic energy to be completely dissipated. This is due particle 
rearrangement at the free surface (\cref{fig:voro_500}). The granular mass 
densifies as the flow progresses, after the initial dilation phase $t = 
20\sqrt{d/g}$.
...
However, the run-out behaviour is expected to depend also on the base friction. 
...
The run-out time, for example, is reduced by a factor of $\approx 4$ as $\mu_s$ 
is increased from 0.1 to 0.2 while the change in the run-out and duration is 
less effected with increase in friction coefficient. This ``saturation effect" 
...
distribution, this energy is even smaller as the initial velocity is distrubted 
...
to a uniform impact velocity. Gradient velocity distribution provides more 
...
due to local destabilisation at the flow front. 
...
properties have a significant influence on the run-out behaviour. 
