...
by the amount of material participating (mobilised) during the collapse. In tall columns, 
the entire column participates in the flow, in contrast to short columns where the 
collapse is due to avalanching of the flanks,~\citet{Lajeunesse2004}. It is possible that 
MPM simulations 
collapses more resulting in longer run-out distance.~\cref{fig:height} shows the 
the normalized final height as a function of the initial aspect ratio of the column. 
Similar to the run-out behaviour, the normalised-height also shows two distinct regimes 
with the initial aspect ratio.The scaling of final height of the column with the initial 
...
for columns with aspect ratio smaller than 0.7. Linear relation between the final height 
and the aspect ratio indicates that only a part of the granular column is active during 
the collapse. For tall columns, both approaches predict similar normalised height. 
However, the normalised height observed in MPM is higher than DEM simulations, which is 
in contrast to the original idea of amount of material participating in the flow. 
