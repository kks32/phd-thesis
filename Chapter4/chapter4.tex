...
`m' (\cref{fig:exp}). The granular column is then released \textit{en 
...
onset of instabilities in a narrow wedges of 56 to 65\si{\degree} for 
...
For tall columns (\textit{a} > 2.7), the flow is still initiated by a well 
defined 
...
material.~\citet{Staron2006} considers 
...
packing density) granular column (\textit{a} = 0.8) are studied to understand 
the 
...
the flow. The dense granular column dilates, while the loose column compacts to 
...
conditions, a higher proportion of the
