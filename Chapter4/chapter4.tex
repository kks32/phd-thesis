...
Comparing the grain scale behaviour with the continuum simulations highlights 
the limitations of the continuum approach in modelling dense granular flows and 
their ability in capturing the complex flow kinematics which are due to 
...
produced~\citep{Abe2013}. The effect of number of material points per cell on 
...
For short columns (`a' < 2.7), the flow is initiated 
...
poly-disperse grains are less susceptible to crystallize especially in the case 
of tall columns. 
...
For tall columns (`a' > 2.7), the flow is still initiated by a well defined 
...
the flow. Whereas in DEM, the initial stage of collapse is characterised by 
...
The energy dissipation mechanism during the collapse provides useful insights 
...
an initial aspect ratio `a' = 0.4. The normalised potential and kinetic energy 
are computed as
%
\begin{align}
...
behaviour is inconsistent with frictional dissipation in continuum modelling of 
granular flow behaviour. 
...
condition, a higher proportion of the
available energy is lost during compaction. This behaviour in addition to 
higher 
mobilised potential energy results in longer run-out distance in dense granular 
...
column flows 
almost twice as much as that of the loose column. Unlike short 
...
results in longer run-out distance for dense granular column in comparison to 
...
system has a significant influence on the flow kinematics 
...
in~\cref{fig:Energy_Time_Slope}. MPM simulations shows two distinct regimes in 
