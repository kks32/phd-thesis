...
which are purely due to micro-structural effects~\citep{Rycroft2009a}. Discrete 
...
%sufficient characterization of the deposit: one also needs to know how mass is 
%distributed during the flow to understand the dynamics and the dissipation 
...
%packing) is studied to understand the role of initial material properties. The 
%evolution of normalised run-out with time for different initial packing 
%density 
%are presented in~\cref{fig:runout_height_dense_r18}. At the initial stage of 
%collapse $t=\tau_c$, the flow behaviour is identical in both dense and loose 
%conditions. However, the dense column flows 30\% longer than the loose 
...
%conditions reveals that there is no change in the mechanism of collapse. The 
%dense condition has slightly higher peak kinetic energy than the loose column. 
%In the free-fall phase, the dense column shows a steeper increase in the 
%horizontal kinetic energy in comparison with the loose column. This indicates 
%that dense granular mass is pushed farther away more quickly than the loose 
%column. Loose column exhibits higher vertical kinetic energy which may be due 
%to particle 
%\begin{figure}[tbhp]
%\centering
...
%\subsection{Summary}
%
%It appears that the collapse experiment for large aspect ratios mixes two very 
%different dynamics: while the second stage consists of a ``conventional''
%horizontal granular flows, the first stage implies a large vertical 
%acceleration. It shows how the initial condition can influence the overall 
%behaviour of a granular system, and suggests that triggering mechanisms play a
%crucial role in the case of natural flows. This stresses the necessity of 
%accounting for vertical acceleration in continuum models in the perspective of 
%producing realistic prediction of the behaviour of granular flows.
%
