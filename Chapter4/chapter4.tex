...
The evolution of the normalized final run-out distance, $\Delta L = 
(L_{\textit{f}}-L_{\textit{0}})/L_{\textit{0}}$, with the initial aspect ratio 
...
In the present study, the following scaling law for the run-out is observed.
\begin{align}
...
\end{cases}
\end{align}
The normalised run-out distance predicted using DEM simulations are very close 
to the run-out observed in the experiments. DEM simulations with hexagonal 
packing estimates lower run-out distances in comparison with randomly packed 
sample. This can be attributed to the crystallisation and jamming effects in 
hexagonal packing. The small difference in the final run-out between DEM and 
experimental results can be attributed to the variation in the 
packing of grains in the granular column. Also, the experimental data 
...
This can cause some variation in the run-out distance.
...
~\citet{Daerr1999} also proposed an interpretation in terms
