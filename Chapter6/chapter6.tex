...
The high permeability granular column shows 
...
\textit{R}. Also with decrease in permeability, the time required to 
...
an 
initial aspect ratio \textit{a} of 0.8 is used. A hydrodynamic radius 
\textit{r} = 0.9 \textit{R} 
...
hydrodynamic radius varied as 0.7, 0.75, 0.8, 0.85 and 0.9 \textit{R}) on the 
run-out 
...
drag. A hydrodynamic radius of \textit{r} = 0.9 \textit{R} is adopted in all 
cases, 
because low 
...
a loosely packed granular column is varied from \textit{r} = 0.7 \textit{R} 
(high 
permeability), 
0.75 \textit{R}, 0.8 \textit{R}, 0.85 \textit{R} to 0.9 \textit{R} (low 
permeability). The run-out distance is 
found to increase with decrease in the permeability of the granular assembly 
(\cref{fig:run5}). The run-out distance for high permeable conditions 
(\textit{r} = 
0.7 \textit{R} -- 0.8 \textit{R}) are lower than their dry counterparts. 
...
With further decrease in permeability (\textit{r} = 0.85 \textit{R} and 0.9 
\textit{R}), the run-out 
...
and 7.5\si{\degree} are studied. A hydrodynamic radius of \textit{r} = 0.85 
\textit{R} 
is 
adopted. 
