...
(see~\cref{fig:permeability}). From the mean flow velocity (\textit{v}), 
...
density of \SI{1000}{\kg$\cdot$\per\cubic\meter}  and a kinematic viscosity of 
\SI{1e-6}{\square\meter$\cdot$\per\second}. The gate supporting the right-hand 
side 
...
of $\Delta t = 2.0E^{-5}$\si{\s} is determined based on the viscosity and 
the 
relaxation 
...
collapse in air condition, both the ``dry'' condition and the 
...
only does it take longer for the pore-pressure to dissipate with decrease 
...
Although the low permeable granular columns take longer duration for the 
...
permeability (see~\cref{fig:Packing_Density_a08_dense}). This drop in the value 
...
a longer run-out distance. Although suspended granular mass experience high 
drag 
...
a difference in the rate of flow evolution. A low permeable column takes longer 
...
material destabilised is small, the flow is thinner and thus has a high 
...
~\cref{fig:Loose_a08_permeability} for low (r = 0.95 R) and high 
permeability conditions (r = 0.9 R). It can be observed that a high 
...
the flow thickness results in higher value of Froude's number and the 
...
significantly high for the high permeability condition at the flow front in 
...
with the surround fluid increases. This increased interaction of the granular 
mass with the surrounding fluid results in the formation of turbulent vortices 
that alters the deposit morphology during the collapse. It was observed 
(see~\cref{sec:collapse_fluid_evolution}) that the 
...
fall is converted into horizontal acceleration. Only during this stage ($t 
...
vortex. The formation of heaps, although is evident, doesn't significantly 
