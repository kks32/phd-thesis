...
$Fr_d$ value of 0.527 is observed for low permeable flow (r = 0.95 R), which 
...
dissipation, over a distance that can be longer than the runout distance in the 
fluid-inertial regime. For short columns, with decrease in permeability the 
run-out distance increases, however, the run-out distance is not higher than 
the dry condition. At higher aspect ratios, decrease in permeability from r = 
0.8 R to r = 0.9 R does not affect the run-out behaviour. This can be 
attributed to the turbulent nature of the granular flows in high aspect ratio 
...
potential energy in the presence of the fluid is dissipated during vertical 
collapse due to viscous friction and contact interactions. In all regimes, the 
...
than the grain-inertial dry column collapse. Also, with decrease in 
permeability the run-out distance increases for the same peak kinetic energy. 
\begin{figure}[tbhp]
	\centering
\begin{subfigure}[b]{0.9\textwidth}
	\centering
	\includegraphics[width=0.9\textwidth]{runout_fluid_dry}
	\caption{Normalised final run-out distance for columns with different 
	initial aspect ratios. Comparison of dry and submerged granular column 
...
\begin{subfigure}[b]{0.9\textwidth}
	\centering
	\includegraphics[width=0.9\textwidth]{runout_fluid_dry_energy}
