...
High permeable column shows lower water entrainment, which indicates that for 
...
The evolution of grain trajectories with time are presented in
...
the behaviour of granular flows down inclined planes (see~\Cref{fig:setup}). 
...
\SI{7.5}{\degree}. The effect of initial packing density is studied using a 
...
experience significant drag forces than the lubrication effect. The difference 
in the 
run-out between the dry and the submerged condition decreases with increase in 
the slope angle. At a slope angle of 5\si{\degree} the difference in the 
run-out 
between 
...
It can be seen that the viscous drag on the dense column tend to predominate 
...
comparison to the dry condition (see~\Cref{fig:KE_dense}). With increase in 
...
(see~\Cref{fig:run_loose}). The run-out distance in fluid increases with 
...
distance (see~\Cref{fig:slope_loose}). For the same thickness of the flow, 
the loose granular flow has smaller density and hence higher Froude's number 
...
granular flows, a plug-like flow can be observed. Due to low permeability, the 
...
This along with the lubrication effect result in longer run-out distance in 
the submerged condition for a loose granular column than the dry collapse. 
...
The evolution of packing density (see~\Cref{fig:voro}) shows that, at the end 
...
in longer run-out distance. The amount of water entrained in the loose granular 
...
for an initially dense granular column. With increase in slope angle, the 
...
results in increase in the difference between the dry and the submerged 
...
slurry state resulting in longer duration of hydroplaning. These effects in the 
loose granular column with low permeability condition result in longer run-out 
distance. 
...
(see~\Cref{fig:run5}). The run-out distance for high permeable conditions (r = 
...
duration of the peak energy, in comparison to dry and high permeable 
conditions (see~\Cref{fig:KE5}). However, the permeability of the granular 
...
column due to lack of viscous dissipation (see~\Cref{fig:height5}).
...
run-out distance in the submerged condition in comparison with the dry granular 
...
density at high permeability conditions. This shows that at high permeability 
...
with the surround fluid increases. This increased interaction of granular mass 
with fluid results in formation of turbulent vortices that alters the deposit 
morphology during the collapse. It was observed 
(see~\cref{sec:collapse_fluid_evolution}) that 
...
radius r = 0.85 R is adopted. Unnatural permeability conditions were observed 
at higher hydrodynamic radii.

The snapshots of the collapse of an aspect ratio 6 column on a horizontal 
...
fall is converted horizontal acceleration. Only during this stage ($t 
= 3\tau_c$) the interactions between the soil grains on the flow surface with 
the fluid result in formation of eddies. The number of eddies formed during the 
...
The snapshots of a collapse of a granular column (\textit{a} = 6) on a inclined 
...
at the flow front, leading to a partial fluidization of the material. Thus 
resulting in longer run-out distance than the dry condition. However, 
